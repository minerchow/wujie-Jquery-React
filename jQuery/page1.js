$(document).ready(function() {
    ...
    // 子应用配置
    const subAppConfig = {
        name: 'react-sub-app-page1',
        url: 'http://localhost:3000/page1', // React 子应用的 Page1 路由
        exec: true,
        // 传递给子应用的初始数据
        props: {
            parentAppName: 'jQuery 父应用 - Page1',
            initialMessage: '来自父应用 Page1 的问候',
            ...
            setTimeout(() => {
                if (window.Wujie && window.Wujie.bus) {
                    window.Wujie.bus.$emit('message-from-parent', {
                        message: '欢迎来到微前端世界！这是来自父应用 Page1 的自动消息。',
    ...
    function stopSubApp() {
        ...
        try {
            ...
            // 清空容器
            $('#subAppContainer').html('<div class="empty-container">React 子应用 page1 将加载到这里...</div>');
        } catch (error) {
