  ...
  useEffect(() => {
    ...
    if (isWujieEnvironment) {
      ...
      if (window.$wujie) {
        ...
        // 向父应用发送初始状态
        window.$wujie.bus.$emit('status-response', {
          status: 'React 子应用 Page2 已加载',
          timestamp: new Date().toISOString()
        });
        
        // 向父应用发送初始消息
        window.$wujie.bus.$emit('message-from-child', {
          message: initialMessage || 'React 子应用 Page2 已启动',
          timestamp: new Date().toISOString()
        });
      }
    }
  ...
  return (
    <div className="page-container">
      ...
      <div style={{ marginTop: '20px', textAlign: 'center', color: '#666' }}>
        <p>这是运行在 Shadow DOM 中的 React 子应用 Page 2</p>
      </div>
    </div>
  );
};
